...
class PySparkCodeReviewer:
    ...
    def _check_performance_issues(self, lines: List[str]) -> List[ReviewIssue]:
        ...
        for i, line in enumerate(lines, 1):
            for pattern_name, pattern_info in self.performance_patterns.items():
                if re.search(pattern_info["pattern"], line, re.IGNORECASE):
                    severity = (
                        "error" if pattern_name == "expensive_operations" else "warning"
                    )
                    issues.append(
                        ReviewIssue(
                            severity=severity,
    ...
    def _generate_metrics(
        self, issues: List[ReviewIssue], lines: List[str]
    ) -> Dict[str, int]:
        ...
        return {
            ...
            "performance_issues": len(
                [i for i in issues if i.category == "performance"]
            ),
            "best_practice_issues": len(
                [i for i in issues if i.category == "best_practice"]
            ),
            "style_issues": len([i for i in issues if i.category == "style"]),
        }

    def generate_report(
        self, issues: List[ReviewIssue], metrics: Dict[str, int]
    ) -> str:
