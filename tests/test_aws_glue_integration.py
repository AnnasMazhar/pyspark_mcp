...
class TestAWSGlueIntegration:
    ...
    def test_generate_glue_job_with_sql_conversion(self):
        ...
        assert (
            result["sql_integration"]["conversion_method"]
            == "integrated_sql_transformation"
        )
    ...
    def test_infer_data_type(self):
        ...
        assert (
            self.glue_integration._infer_data_type("2023-01-15T10:30:00") == "timestamp"
        )
        assert (
            self.glue_integration._infer_data_type("2023-01-15 10:30:00") == "timestamp"
        )
    ...
    def test_is_likely_partition_key(self):
        ...
        assert (
            self.glue_integration._is_likely_partition_key("partition_date", records)
            is True
        )

        # Test with low cardinality
        records = [{"region": "US"} for _ in range(10)] + [
            {"region": "EU"} for _ in range(5)
        ]
        ...
        assert (
            self.glue_integration._is_likely_partition_key("user_id", records) is False
        )

    def test_get_serde_configurations(self):
        """Test SerDe configuration generation."""
        # Test Parquet
        assert "ParquetHiveSerDe" in self.glue_integration._get_serde_library(
            DataFormat.PARQUET
        )
        assert self.glue_integration._get_serde_parameters(DataFormat.PARQUET) == {}

        # Test CSV
        assert "LazySimpleSerDe" in self.glue_integration._get_serde_library(
            DataFormat.CSV
        )
    ...
    def test_generate_schema_recommendations(self):
        ...
        string_columns = [
            ColumnInfo(name=f"str_col_{i}", type="string") for i in range(10)
        ]
        recommendations = self.glue_integration._generate_schema_recommendations(
            string_columns, [], []
        )
    ...
    def test_get_default_value_for_type(self):
        ...
        assert (
            self.glue_integration._get_default_value_for_type("array<string>") == "[]"
        )
        assert self.glue_integration._get_default_value_for_type("struct<>") == "{}"
        assert (
            self.glue_integration._get_default_value_for_type("unknown_type") == "None"
        )
    ...
    def test_analyze_s3_optimization_opportunities(self):
        ...
        assert any(
            "date-based partitioning" in rec
    ...
    def test_incremental_best_practices(self):
        ...
        timestamp_practices = (
            self.glue_integration._generate_incremental_best_practices("timestamp")
        )
        watermark_practices = (
            self.glue_integration._generate_incremental_best_practices("watermark")
        )
        bookmark_practices = self.glue_integration._generate_incremental_best_practices(
            "bookmark"
        )
        ...
        assert any(
            "timezone conversions" in practice for practice in timestamp_practices
        )
        assert any("watermark table" in practice for practice in watermark_practices)
        assert any(
            "bookmark limitations" in practice for practice in bookmark_practices
        )
