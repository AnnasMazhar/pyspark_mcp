...
class TestDataSourceAnalyzer:
    ...
    def test_infer_format_from_path(self):
        ...
        for path, expected_format in test_cases:
            ...
            assert result == expected_format, f"Expected {expected_format} for {path}, got {result}"
...
"""
        ...
        s3_sources = [ds for ds in analysis['data_sources'] if ds.source_type == 'file' and 's3://' in ds.location]
        hive_sources = [ds for ds in analysis['data_sources'] if ds.source_type == 'hive']
        assert len(s3_sources) >= 1
        assert len(hive_sources) >= 1
        
        # Check performance issues
        collect_issues = [issue for issue in analysis['performance_issues'] if '.collect()' in issue]
...
""")
    ...
    def test_get_recommendations_with_codebase_analysis(self):
        ...
        recommendations = self.analyzer.get_recommendations(data_sources, codebase_analysis)
...
""")
            ...
            # Check issues
            assert len(analysis.performance_issues) >= 2  # collect() and count() without cache
