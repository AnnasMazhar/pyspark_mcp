...
class TestOutputManager:
    ...
    def test_generate_output_filename(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            filename = manager.generate_output_filename(
                "query.sql", query_index=1, timestamp=False
            )
...
class TestSecurityFeatures:
    ...
    def test_secure_output_manager(self, temp_dir):
        ...
        safe_filename = manager.generate_output_filename(
            "normal_file.sql", timestamp=False
        )
        assert safe_filename == "normal_file.py"

        # Test dangerous filename sanitization
        dangerous_filename = manager.generate_output_filename(
            "../../../etc/passwd", timestamp=False
        )
