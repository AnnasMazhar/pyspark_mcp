...
"""
    ...
    def test_analyze_data_flow_basic(self):
        ...
        analysis = self.optimizer.analyze_data_flow(
    ...
    def test_analyze_data_flow_with_table_info(self):
        ...
        read_nodes = [
            node for node in analysis.nodes.values() if node.operation == "read"
        ]
    ...
    def test_suggest_partitioning_strategy(self):
        ...
        strategies = self.optimizer.suggest_partitioning_strategy(
            analysis, self.sample_table_info
        )
    ...
    def test_recommend_join_strategy(self):
        ...
        optimizations = self.optimizer.recommend_join_strategy(
            analysis, self.sample_table_info
        )
        ...
        if optimizations:  # If optimizations were generated
            ...
            assert opt.recommended_strategy in [
                "broadcast",
                "sort_merge",
                "shuffle_hash",
            ]
...
"""
        ...
        if optimizations:
            ...
            broadcast_opts = [
                opt for opt in optimizations if opt.recommended_strategy == "broadcast"
            ]
...
"""
        ...
        caching_recs = [
            ...
            if rec.optimization_type == OptimizationType.CACHING
        ]
...
"""
        ...
        pruning_recs = [
            rec
            for rec in recommendations
            if rec.optimization_type == OptimizationType.COLUMN_PRUNING
        ]
        ...
        has_select_star = any(
            re.search(pattern, select_star_code, re.IGNORECASE)
...
"""
        ...
        coalesce_recs = [
            rec
            for rec in recommendations
            if rec.optimization_type == OptimizationType.COALESCE
        ]
    ...
    def test_table_size_estimation(self):
        ...
        size_with_info = self.optimizer._estimate_table_size(
            "customers", self.sample_table_info
        )
    ...
    def test_filter_selectivity_estimation(self):
        ...
        equality_selectivity = self.optimizer._estimate_filter_selectivity(
            "col('id') == 123"
        )
        range_selectivity = self.optimizer._estimate_filter_selectivity(
            "col('amount') > 1000"
        )
        like_selectivity = self.optimizer._estimate_filter_selectivity(
            "col('name').like('%test%')"
        )
    ...
    def test_optimization_priority_sorting(self):
        ...
        if len(recommendations) > 1:
            ...
            high_priority_indices = [
                i for i, rec in enumerate(recommendations) if rec.priority == "high"
            ]
            medium_priority_indices = [
                i for i, rec in enumerate(recommendations) if rec.priority == "medium"
            ]
    ...
    def test_join_size_estimation_logic(self):
        ...
        optimization = self.optimizer._analyze_join_optimization(
            join_op, analysis, None
        )
...
class TestOptimizationTypes:
    ...
    def test_all_optimization_types_covered(self):
        ...
        for opt_type in OptimizationType:
            ...
            assert opt_type in optimizer.performance_models or opt_type.value in [
                "column_pruning",
                "predicate_pushdown",
                "coalesce",
                "repartition",
